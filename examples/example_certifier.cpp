...
#include "IterCertAlg/MinNormCert.h"
#include "IterCertAlg/MinNormCert.cpp"
...
int main(int argc, char** argv)
{
  ...
  std::srand(std::time(nullptr));
      ...
      */
      ...
      Eigen::Matrix<double, 9, r_left> Ured = U.block<9,r_left>(0,4); 
      Matrix5 Dleft = (eig_left.eigenvalues().block<r_left,1>(4,0)).cwiseAbs().cwiseSqrt().asDiagonal();
      Yleft = Ured * Dleft;
      
      std::cout << "Init:\n" << Yleft << std::endl;
      RankYCert::RankYCertOptions options_ranky;
      options_ranky.max_RTR_iterations = 20;
      
      RankYCert::RankYCertClass<Matrix9, Eigen::Matrix<double, 9, r_left>, Vector9, Vector7> cert_iter_ranky(options_ranky);
     
      RankYCert::RankYCertResult<Matrix9, Eigen::Matrix<double, 9, r_left>, Vector7> res_left_ranky = cert_iter_ranky.getResults(x_left, 
                                                                                                            C, Aleft, res_left.opt_mult, Yleft);
      
    
      /* Try minimum norm */
      MinNormCert::MinNormCertClass<Matrix9, Vector9, Vector7> cert_iter_min;
     
      MinNormCert::MinNormCertResult<Matrix9, Vector9, Vector7> res_left_min = cert_iter_min.getResults(x_left, C, Aleft, mult_left_init);
      
      
      
      Eigen::SelfAdjointEigenSolver<Matrix9> eig_left_min(res_left_min.opt_Hessian);
      std::cout << "Eigenvalues Hessian LEFT:\n" << eig_left_min.eigenvalues() << std::endl; 
      
      std::cout << "Multipliers:\n" << res_left_min.opt_mult << std::endl; 
      
      
      
      Yleft.setZero(); 
      U = eig_left_min.eigenvectors();  
      Ured = U.block<9,r_left>(0,4); 
      Dleft = (eig_left_min.eigenvalues().block<r_left,1>(4,0)).cwiseAbs().cwiseSqrt().asDiagonal();
      Yleft = Ured * Dleft;
      
      
      RankYCert::RankYCertClass<Matrix9, Eigen::Matrix<double, 9, r_left>, Vector9, Vector7> cert_iter_ranky_min(options_ranky);
     
      RankYCert::RankYCertResult<Matrix9, Eigen::Matrix<double, 9, r_left>, Vector7> res_left_ranky_min = cert_iter_ranky.getResults(x_left, 
                                                                                                             C, Aleft, res_left_min.opt_mult, Yleft);
      ...
      // check feasibility sol
      // check independence constraints
      cert_iter_ranky.checkFeasibilitySol(x_left, Aleft); 
      cert_iter_ranky.checkConstIndp(Aleft); 
      ...
      RankYCert::RankYCertResult<Matrix9, Eigen::Matrix<double, 9, r_right>, Vector7> res_right_ranky = cert_iter_ranky.getResults(x_right, 
                                                                                                          C, Aright, mult_right_init, Yright);
       
             MinNormCert::MinNormCertResult<Matrix9, Vector9, Vector7> res_right_min = cert_iter_min.getResults(x_right, C, Aright, mult_right_init);
      ...
      RankYCert::RankYCertClass<Matrix12, Eigen::Matrix<double, 12, r_both>, Vector12, Vector13> cert_iter_ranky_both(options_ranky);
     
      RankYCert::RankYCertResult<Matrix12, Eigen::Matrix<double, 12, r_both>, Vector13> res_both_ranky = cert_iter_ranky_both.getResults(x_both, 
                                                                                                          C_ext, Aboth, mult_both_init, Yboth);
                                                                   
                                                                   
      
      MinNormCert::MinNormCertClass<Matrix12, Vector12, Vector13> cert_iter_max;                                                                                                      
      MinNormCert::MinNormCertResult<Matrix12, Vector12, Vector13> res_both_max = cert_iter_max.getResults(x_both, C_ext, Aboth, mult_both_init);                                                                                                  
      ...
      std::cout << "norm eigenvalues:\n" << eig_adj.eigenvalues() / eig_adj.eigenvalues()(11) << std::endl;
      
      const int r_adj = 5;
      Eigen::Matrix<double, 12, r_adj> Yadj; 
      Matrix12 U_adj = eig_adj.eigenvectors();  
      Eigen::Matrix<double, 12, r_adj> Ured_adj = U_adj.block<12,r_adj>(0,7); 
      Matrix5 Dadj = (eig_adj.eigenvalues().block<r_adj,1>(7,0)).cwiseAbs().cwiseSqrt().asDiagonal();
      Yadj = Ured_adj * Dadj;
      
      RankYCert::RankYCertClass<Matrix12, Eigen::Matrix<double, 12, r_adj>, Vector12, Vector22> cert_iter_ranky_adj(options_ranky);
     
      RankYCert::RankYCertResult<Matrix12, Eigen::Matrix<double, 12, r_adj>, Vector22> res_adj_ranky = cert_iter_ranky_adj.getResults(x_adj, 
                                                                                                        C_ext, Aadj, mult_adj_init, Yadj);
                                                                                                          
      
      MinNormCert::MinNormCertClass<Matrix12, Vector12, Vector22> cert_iter_max2;                                                                                                      
      MinNormCert::MinNormCertResult<Matrix12, Vector12, Vector22> res_both_max2 = cert_iter_max2.getResults(x_adj, C_ext, Aadj, mult_adj_init);  
